...
| [`P1.png`](./P1.png) | **P1 地域ビュー** — 今週の顔ぶれTop10（順位=latest_rank、バー=ランクイン週数、NEWバッジ）／時系列（52週）／今週Top25明細（フル幅）の3ブロック。※旧「地元ワード＋急浮上」ブロックは廃止 |
| [`P1-wireframe.html`](./P1-wireframe.html) / [`P1-wireframe.png`](./P1-wireframe.png) | P1 再構成後のレイアウト図（ブロック構成・使用データソース・廃止内容のメモ。Looker 構築時の設計確認用） |
...
chromium-browser --headless=new --disable-gpu --hide-scrollbars \
  --window-size=1280,2260 --screenshot=demos/google-trends/mock/P1.png "file://$PWD/$SRC"
